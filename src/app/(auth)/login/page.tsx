...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
      toast.error("Geli iimayl iyo furaha");
      return;
    }
    setLoading(true);
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Gelitaanku wuu fashilmay");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex min-h-screen flex-col bg-white">
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-6 py-12">
        <div className="mb-10 flex flex-col items-center text-center">
          ...
          <h1 className="text-[22px] font-bold tracking-tight text-slate-900">{t.welcomeBack}</h1>
          <p className="mt-1 text-sm text-slate-500">{t.signInContinue}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-3">
          <input type="email" placeholder={t.emailOrUsername} value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" required
          ...
          <input type="password" placeholder={t.password} value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" required
            className="h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-[15px] text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
          <div className="flex justify-end pt-0.5">
            <Link href="/forgot-password" className="text-sm font-medium text-blue-600 hover:underline">{t.forgotPassword}</Link>
          </div>
        ...
        <p className="mt-8 text-center text-sm text-slate-500">
          {t.noAccount}{" "}
          <Link href="/register" className="font-semibold text-blue-600 hover:underline">{t.signUp}</Link>
        </p>

        <button type="button" onClick={() => { enterDemo(); router.push("/home"); }} className="mt-5 text-center text-xs text-slate-400 hover:text-slate-600">
          {t.tryDemo}
        </button>
      </div>
    </div>
