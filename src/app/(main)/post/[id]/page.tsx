          ...
          {comments.map((c) => (
            <div key={c.id} className="flex gap-2.5">
              <Link href={`/u/${c.authorId}`}>
                <Avatar src={c.authorAvatar} alt={c.authorNickname} size="sm" />
              </Link>
              <div className="min-w-0 flex-1">
                <div className="rounded-2xl bg-slate-100 px-3 py-2">
                  <Link href={`/u/${c.authorId}`} className="text-xs font-semibold text-slate-900 hover:underline">
                    {c.authorNickname}
                  </Link>
                  <p className="text-sm text-slate-800">{c.content || c.text}</p>
                </div>
        ...
        <div className="fixed bottom-16 left-0 right-0 z-40 border-t border-slate-100 bg-white px-3 py-2 md:bottom-0">
          <div className="mx-auto flex max-w-2xl items-center gap-2">
            ...
            <input
              ...
              className="h-10 flex-1 rounded-full border border-slate-200 bg-slate-50 px-4 text-sm focus:border-blue-500 focus:outline-none"
            />
            <button
              type="button"
