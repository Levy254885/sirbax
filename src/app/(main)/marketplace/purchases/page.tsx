...
export default function PurchasesPage() {
  ...
  useEffect(() => {
    if (!user) return;
    listDealsForUser(user.uid).then((all) => setItems(all.filter((d) => d.buyerId === user.uid)));
  }, [user?.uid]);
  return (
    <AppShell showRight={false}>
      <MarketplaceHeader />
      <div className="mx-auto max-w-2xl px-4 py-4 pb-28">
        <h1 className="mb-4 text-lg font-bold">My purchases</h1>
        {items.length === 0 && <p className="py-12 text-center text-sm text-slate-400">No purchases yet.</p>}
